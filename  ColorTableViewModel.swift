...
class ColorTableViewModel: NSObject {
    ...
    init (view:ColorListTableViewControllerDelegate?, title:String) {
        ...
        self.coulorData = []
        if let redModel = Color(name: "Red", red: 1.0, green: 0.0, blue:
            0.0, alpha: 1.0),
            let blueModel = Color(name: "Blue", red: 0.0, green: 0.0, blue:
                1.0, alpha: 1.0),
            let greenModel = Color(name: "Green", red: 0.0, green: 1.0,
                                   blue: 0.0, alpha: 1.0) {
...
extension ColorTableViewModel : TableViewModel {
    ...
    func viewModelForSelectedRow() -> ColorDetailViewModel? {
        guard let selectedIndexPath = selectedIndexPath else {
            return nil }
        if selectedIndexPath.row < 0 || selectedIndexPath.row >= coulorData.
        count {
            return nil }
        return ColorDetailViewModel(view:nil, model:coulorData[selectedIndex
            Path.row]) }
